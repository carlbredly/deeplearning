...
class PINNs:
    ...
    def __init__(self, X_colloc, net_transform, net_pde_user, loss_f, layers, lr, param_pde=None,
        ...
        if X_bc is None:
            ...
            self.X_bc = keras.ops.convert_to_tensor(X_bc, dtype='float64')
            self.u_bc = keras.ops.convert_to_tensor(u_bc, dtype='float64')
        ...
        if X_init is None:
            ...
            self.X_init = keras.ops.convert_to_tensor(X_init, dtype='float64')
            self.u_init = keras.ops.convert_to_tensor(u_init, dtype='float64')
        ...
        if X_data is None:
            ...
            self.X_data = keras.ops.convert_to_tensor(X_data, dtype='float64')
            self.u_data = keras.ops.convert_to_tensor(u_data, dtype='float64')
        ...
        if X_other is None:
            ...
            self.X_other = keras.ops.convert_to_tensor(X_other, dtype='float64')
            self.u_other = keras.ops.convert_to_tensor(u_other, dtype='float64')
        ...
        if X_test is None:
            ...
            self.X_test = keras.ops.convert_to_tensor(X_test, dtype='float64')
            self.u_test = keras.ops.convert_to_tensor(u_test, dtype='float64')
        ...
        if X_traction is None:
            ...
            self.X_traction = keras.ops.convert_to_tensor(X_traction, dtype='float64')
            self.nb_traction = self.X_traction.shape[0]

        self.X_colloc = keras.ops.convert_to_tensor(X_colloc, dtype='float64')
        ...
        if self.type_problem =='inverse':
            ...
            if param_pde is not None:
                self.param_pde = keras.ops.convert_to_tensor(param_pde, dtype='float64')
                self.nb_param = self.param_pde.shape[0]
            else:
                self.param_pde = None
                self.nb_param = 1
        ...
        if self.model_init is None:
            if self.type_formulation == 'weak':
                self.net_u = keras.Sequential()
                self.net_u.add(keras.layers.InputLayer(shape=(self.layers[0],)))
                for i in range(1, len(self.layers) - 1):
                    self.net_u.add(
                        keras.layers.Dense(self.layers[i], activation=keras.activations.tanh,
                                           kernel_initializer=keras.initializers.RandomNormal(mean=0, stddev=0.1),
                                              bias_initializer='zeros'))  # "glorot_normal"
                self.net_u.add(keras.layers.Dense(self.layers[-1], activation=None,
                ...
                self.net_u.add(
                    keras.layers.Dense(self.layers[-1], activation=None, kernel_initializer="glorot_normal"))
        else:
            self.net_u = keras.Sequential()
            self.net_u.add(keras.layers.InputLayer(shape=(self.model_init.layers[0].get_weights()[0].shape[0],)))
            for i in range(1, len(self.model_init.layers)):
                ...
                self.net_u.add(keras.layers.Dense(self.model_init.layers[i].get_weights()[0].shape[0], activation=self.activation,
            ...
            self.net_u.add(keras.layers.Dense(self.model_init.layers[-1].get_weights()[0].shape[0], activation=None,
                                                 kernel_initializer=keras.initializers.Constant(W),
                                                 bias_initializer=keras.initializers.Constant(b)))

        self.tf_optimizer = keras.optimizers.Adam(learning_rate=lr)
    ...
    def loss_pinns(self, X_f, param_f, model_nn, u_pred_bc, u_star_bc, u_pred_init, u_star_init, u_pred_data,
        ...
        #        f_value += keras.ops.mean(keras.ops.square(f[i]))
        loss_obs = 0.0
        loss_bc = 0.0
        loss_init = 0.0
        loss_data = 0.0
        ...
        if self.type_problem!='generalization':
            ...
            if self.nb_bc > 0:
                for i in range(u_star_bc.shape[1]):
                    if not keras.ops.isnan(u_star_bc[0, i:(i+1)]):
                        loss_bc += keras.ops.mean(keras.ops.square(u_pred_bc[:, i:(i + 1)] - u_star_bc[:, i:(i + 1)]))
                        loss_obs += keras.ops.mean(keras.ops.square(u_pred_bc[:, i:(i+1)] - u_star_bc[:, i:(i+1)]))
                    else:
                        loss_bc +=  keras.ops.convert_to_tensor(0, dtype='float64')
                        loss_obs +=  keras.ops.convert_to_tensor(0, dtype='float64')

            if self.nb_init > 0:
                for i in range(u_star_init.shape[1]):
                    if not keras.ops.isnan(u_star_init[0, i:(i+1)]):
                        loss_init += keras.ops.mean(keras.ops.square(u_pred_init[:, i:(i + 1)] - u_star_init[:, i:(i + 1)]))
                        loss_obs += keras.ops.mean(keras.ops.square(u_pred_init[:, i:(i+1)] - u_star_init[:, i:(i+1)]))
                    else:
                        loss_init +=  keras.ops.convert_to_tensor(0, dtype='float64')
                        loss_obs +=  keras.ops.convert_to_tensor(0, dtype='float64')
            if self.nb_data > 0:
                for i in range(u_star_data.shape[1]):
                    if not keras.ops.isnan(u_star_data[0, i:(i+1)]):
                        loss_data += keras.ops.mean(keras.ops.square(u_pred_data[:, i:(i + 1)] - u_star_data[:, i:(i + 1)]))
                        loss_obs += keras.ops.mean(keras.ops.square(u_pred_data[:, i:(i + 1)] - u_star_data[:, i:(i + 1)]))
                    else:
                        loss_data +=  keras.ops.convert_to_tensor(0, dtype='float64')
                        loss_obs +=  keras.ops.convert_to_tensor(0, dtype='float64')
            if self.nb_other >0:
                for i in range(u_star_other.shape[1]):
                    if not keras.ops.isnan(u_star_other[0, i:(i+1)]):
                        loss_other += keras.ops.mean(keras.ops.square(u_pred_other[:, i:(i + 1)] - u_star_other[:, i:(i + 1)]))
                        loss_obs += keras.ops.mean(keras.ops.square(u_pred_other[:, i:(i+1)] - u_star_other[:, i:(i+1)]))
                    else:
                        loss_other +=  keras.ops.convert_to_tensor(0, dtype='float64')
                        loss_obs +=  keras.ops.convert_to_tensor(0, dtype='float64')
        else:
            for i_param in range(self.nb_param):
                if self.nb_bc > 0:
                    ...
                    for i in range(u_star_bc.shape[1]):
                        if not keras.ops.isnan(u_star_bc[size_bc * i_param:size_bc * (i_param + 1), i:(i+1)][0]):
                            loss_obs += keras.ops.mean(keras.ops.square(
                ...
                if self.nb_init > 0:
                    ...
                    for i in range(u_star_init.shape[1]):
                        if not keras.ops.isnan(u_star_init[size_init * i_param:size_init * (i_param + 1), i:(i + 1)][0]):
                ...
                if self.nb_data > 0:
                    ...
                    for i in range(u_star_data.shape[1]):
                        if not keras.ops.isnan(u_star_data[size_data * i_param:size_data * (i_param + 1), i:(i + 1)][0]):
                            loss_obs += keras.ops.mean(keras.ops.square(
                ...
                if self.nb_other > 0:
                    ...
                    for i in range(u_star_data.shape[1]):
                        if not keras.ops.isnan(u_star_other[size_other * i_param:size_other * (i_param + 1), i:(i + 1)][0]):
                            loss_obs += keras.ops.mean(keras.ops.square(
                ...
                index_i_param = keras.ops.where(X_f[:, -1] == param_f[i_param])
                index_i_param = keras.ops.reshape(index_i_param, [-1])
                f_i = keras.ops.take(f, index_i_param)
    ...
    def test_pde(self, X_sup_test, u_sup_test, model_test):
        ...
        return keras.ops.mean(
            keras.ops.square(u_pred_test - u_sup_test)) / keras.ops.mean(keras.ops.square(u_sup_test))
    ...
    def train(self, max_epochs_adam=0, max_epochs_lbfgs=0, print_per_epochs=1000):
        ...
        def optimizer_lbfgs(X_f, param_f, method='L-BFGS-B', **kwargs):
            ...
            def get_weight():
                ...
                list_weight = keras.ops.convert_to_tensor(list_weight)
                return list_weight
            ...
            def set_weight(list_weight):
                ...
                for variable in self.pinns_training_variables():
                    if len(variable.shape) == 2:
                        ...
                        new_variable = keras.ops.reshape(list_weight[index:index + len_weights],
                        ...
                        index += 1
                    variable.assign(keras.ops.cast(new_variable, 'float64'))
